<?php
 include('connection.php');
  ...
  else if($_SESSION['UID'] == $_GET['userid'])
  {
    header('Location: myprofile.php');
  }

  $res = $mysqli->query("SELECT * FROM Users WHERE user_id = ".$_GET['userid']."") or die($mysql->error);
  $res->data_seek(0);
  $row = $res->fetch_assoc();

  $albumq = $mysqli->query("SELECT a.name, a.album_id FROM Albums a, Users u WHERE a.user_id = u.user_id AND a.user_id=".$_GET['userid']."") or die($mysqli->error);
  $friendq = $mysqli->query("SELECT u.first_name,u.user_id FROM Friends f, Users u WHERE f.user_fid = u.user_id AND f.user_id=".$_GET['userid']."") or die($mysqli->error);
?>
<!DOCTYPE html>
<html>
  <head>
  ...
  <body>
    ...
    <div id="body" style="color:black;background:rgba(255,255,255, 0.8);">
      ...
      <?php
        ...
        if($_SESSION['Login'] == 'Yes')
        {
          $ch = $mysqli->query("SELECT * FROM Friends where user_id=".$_SESSION['UID']." AND user_fid=".$_GET['userid'])or die($mysqli->error);
          if($ch->num_rows == 0)
            echo "<a href='addfriend.php?fid=".$row['user_id']."'>Add Friend</a><br>";
          else if($ch->num_rows == 1)
        ...
        for($i = 0; $i<$albumq->num_rows;$i=$i+1)
        {
          ...
          //var_dump($r);
          echo "&nbsp;<a href='viewalbum.php?albumid=".$r['album_id']."&albumname=".$r['name']."'>".$r['name']."</a>&nbsp;|";
        }
      ?>
      &nbsp;
      <br>
      <?php
        ...
        echo "<h2 style='display:inline-block;'><br><br>FRIENDS WITH:&nbsp;&nbsp;</h2>";
